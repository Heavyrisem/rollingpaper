...
Server.get("/GetNotes", async (req, res) => {
  ...
  let HeaderNote: DB_Note | null = await DB.collection("Notes").findOne({
    header: true,
  });
...
Server.listen(6800, async () => {
  ...
  DB_Client = await mongo.connect(
    `mongodb://${process.env.user}:${process.env.pwd}@${process.env.host}/${process.env.DataBase}`,
    DB_Config
  );
